...
(function() {
    ...
    this.addIssuesToEpic = function(issueKeys, epicKey, callback) {
        var options = {
            rejectUnauthorized: this.strictSSL,
            uri: this.makeUri('/epics/' + epicKey + '/add/', 'rest/greenhopper/', '1.0'),
            method: 'PUT',
            json: true,
            body: {
              issueKeys: issueKeys
        ...
        this.doRequest(options, function(error, response) {
            ...
            callback(response.statusCode + ': Error while adding issues to sprint');
        });
    };
